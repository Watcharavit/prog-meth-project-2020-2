...
public class BombCenter extends BombFlame implements Updatable {
	...
	public void update(double ticksPassed) {
		...
		if (newExplodeRadius >= 1 && newExplodeRadius > explodeRadius) {
			for (int i = explodeRadius + 1; i <= newExplodeRadius; i++) this.explodeForRadius(i);
			explodeRadius = newExplodeRadius;
		}
	}
	
	private boolean explodeAt(int x, int y, double lifetime) {
		if (x < 0 || x >= GameSingleton.WIDTH || y < 0 || y >= GameSingleton.HEIGHT) return true;
	...
	private void explodeForRadius(int radius) {
		...
		if (!lStop) lStop = this.explodeAt(x - radius, y, lifetime);
		if (!rStop) rStop = this.explodeAt(x + radius, y, lifetime);
		if (!uStop) uStop = this.explodeAt(x, y - radius, lifetime);
		if (!dStop) dStop = this.explodeAt(x, y + radius, lifetime);
	}
