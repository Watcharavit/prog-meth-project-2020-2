...
public class EquipmentsGranterDaemon extends PhantomEntity implements Updatable {
	...
	public void update(double ticksPassed) {
		...
		for (Player player : players) {
			...
			if (kingTime > topPlayer.getKingTime()) topPlayer = player;
			if (kingTime < bottomPlayer.getKingTime()) bottomPlayer = player;			
		}
		if (topPlayer != bottomPlayer) {
			if (topPlayer != lastTopPlayer && lastTopPlayer != null) lastTopPlayer.setEquipment(null);
			if (bottomPlayer != lastBottomPlayer && lastBottomPlayer != null) lastBottomPlayer.setEquipment(null);
			if (topPlayer != lastTopPlayer) topPlayer.setEquipment(new EquipmentBombKicker(topPlayer));
			if (bottomPlayer != lastBottomPlayer) bottomPlayer.setEquipment(new EquipmentPuncher(bottomPlayer));
