...
class GameplayManager {
	protected static void setupGameplay(Pane uiPane) {
		...
		GameSingleton.addPhantomEntity(new GameTimerDaemon(players, timerUiPane));
	...
	static class PlayersAdder {
		...
		protected static Player[] createPlayers(Pane uiPane) {
			...
			Player[] allPlayers = {
					...
					createPlayer("Player 2", player2Control, GameSingleton.tiles[GameSingleton.WIDTH - 3][GameSingleton.HEIGHT - 3],
		...
		private static Player createPlayer(String name, Map<PlayerControl, KeyCode> keyMap, Tile spawnTile, Sprite normalSprite,
				Sprite dyingSprite, VBox containingUiBox) {
